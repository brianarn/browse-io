...
function BrowseIOServer (opts) {
  ...
  io.on('connection', function (socket) {
    ...
    events.on('board-details', function (details) {
      socket.emit('board-details', details);
    });

    events.on('board-event', throttle(function (event) {
      socket.volatile.emit('board-event', event);
    }, 50));
